...
interface UserTableProps {
    users: {
        ...
        encryptedId?: string;
        lists?: { id: string; name: string; _count?: { items: number } }[];
        _count?: {
            userMedia: number;
            lists: number;
        };
                ...
                {users.map((user, index) => (
                    <div key={user.id} className="rounded-xl border border-white/5 bg-zinc-900/50 p-4 space-y-4">
                        ...
                        <div className="flex gap-3 pt-1">
                            ...
                            {user._count?.userMedia && user._count.userMedia > 0 ? (
                                <Link
                                ...
                                <span className="flex-1 flex items-center justify-center gap-2 bg-zinc-800/30 py-2 rounded-lg text-xs text-zinc-400 border border-white/5">No media</span>
                            )}
                            {user._count?.lists && user._count.lists > 0 ? (
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <button className="flex-1 flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 py-2 rounded-lg text-xs font-medium transition-colors border border-white/5 cursor-pointer">
                                    ...
                                    <DropdownMenuContent align="end" className="w-48 bg-zinc-900 border-white/10">
                                        <DropdownMenuLabel className="text-zinc-500">{user.name} lists</DropdownMenuLabel>
                                        <DropdownMenuSeparator className="bg-white/5" />
                                        {user.lists?.map((list) => (
                                            <DropdownMenuItem key={list.id} asChild className="focus:bg-white/5 focus:text-white cursor-pointer group/item">
                                                <Link href={getLocalizedPath(`/sharelist/${user.encryptedId || user.id}?category=list_${list.id}`, locale)} className="w-full flex items-center justify-between gap-4">
                                                    <span className="truncate">{list.name}</span>
                                                    <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-zinc-500 group-hover/item:text-zinc-300 transition-colors shrink-0">
                                                        {list._count?.items || 0}
                                                    </span>
                                                </Link>
                                            </DropdownMenuItem>
                                ...
                                <td className="p-4">
                                    <div className="flex items-center flex-wrap justify-center gap-4 text-sm">
                                        ...
                                        {user._count?.lists && user._count.lists > 0 ? (
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <button className="flex items-center gap-1.5 text-zinc-300 hover:text-white transition-colors cursor-pointer group">
