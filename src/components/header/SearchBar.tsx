    ...
    return (
        <>
            <button
                ...
                className='flex w-fit sm:w-[120px] md:w-[180px] lg:w-[150px] xl:w-fit items-center gap-2 px-2 py-1 sm:px-3 sm:py-1.5 sm:mr-3 md:mr-5 rounded-lg bg-zinc-900/50 border border-white/5 hover:border-white/20 transition-all text-zinc-400 hover:cursor-text hover:scale-115 duration-500'
            >
                <Search size={18} />
                <span className='text-[16px] hidden sm:inline-block hover:cursor-text w-full truncate whitespace-nowrap font-medium'>
                    {t('SearchBar.searchPlaceholder').slice(0, 30)}...
                </span>
            </button>
                                ...
                                <div className='w-full bg-zinc-900/40 border border-white/5 rounded-xl max-h-[80vh] sm:max-h-[60vh] flex flex-col shadow-3xl backdrop-blur-xl'>
                                    {isLoading && debouncedSearch !== '' ? (
                                        <div className='p-20 flex flex-col items-center justify-center gap-4 text-zinc-500'>
                                            ...
                                            <span className='font-black uppercase tracking-widest text-[10px]'>{t('loading')}</span>
                                        </div>
                                    ) : results.length > 0 ? (
                                                            ...
                                                            <div className='flex-1 flex flex-col gap-1 min-w-0'>
                                                                <div className='flex items-center gap-2'>
                                                                    <span className='text-[10px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded bg-white/5 text-zinc-500'>
                                                                        {t(result.media_type as "TV" | "Movie" | "Person")}
                                                                    </span>
                                        ...
                                        <div className='p-20 flex flex-col items-center justify-center gap-4 text-zinc-600'>
                                            ...
                                            <span className='font-black uppercase tracking-widest text-[10px]'>{t('SearchBar.noResults')} "{debouncedSearch}"</span>
                                        </div>
                                    ) : (
                                        <div className='p-5 sm:p-20 flex flex-col items-center justify-center gap-4 text-zinc-700'>
                                            ...
                                            <p className='text-sm font-medium'>{t('SearchBar.searchText')}</p>
                                            <span className='text-[10px] font-black uppercase tracking-widest opacity-30'>{t('SearchBar.movies')} • {t('SearchBar.series')} • {t('SearchBar.actors')}</span>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
